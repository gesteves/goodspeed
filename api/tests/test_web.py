...
def test_healthz_200_includes_field_cycle_when_field_published(
    client: TestClient, tmp_path: Path
):
    _write_point_feed(tmp_path, cycle="2026-05-22T21:00:00Z")
    body = json.dumps({"model": {"cycle": "2026-05-22T21:00:00Z"}, "frames": []}).encode()
    storage.push_field_feed(body, tmp_path)
    r = client.get("/healthz")
    assert r.status_code == 200
    payload = r.json()
    assert payload["field_status"] == "ok"
    assert payload["field_cycle"] == "2026-05-22T21:00:00Z"


def test_healthz_500_when_point_feed_corrupted(client: TestClient, tmp_path: Path):
    # Bypass push_feed so we can write invalid JSON.
    (tmp_path / storage.LATEST_KEY).write_bytes(b"{this is not json")
    r = client.get("/healthz")
    assert r.status_code == 500
    payload = r.json()
    assert payload["status"] == "broken"
    assert payload["error"]  # error string captured
    assert r.headers["cache-control"] == "no-store"

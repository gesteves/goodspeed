...
def test_read_published_cycle_roundtrip(tmp_path: Path):
    ...
    storage.push_feed(body, cycle_iso, out_dir=tmp_path)
    assert storage.read_published_cycle(out_dir=tmp_path) == cycle_iso


def test_read_published_cycle_missing(tmp_path: Path):
    """No latest.json yet → None, so the caller fetches."""
    assert storage.read_published_cycle(out_dir=tmp_path) is None
